<template>
  <div class="row mt-5 pt-2" v-if="order">
    <div class="col-lg-6">
      <div class="row">
        <div v-for="item in images" :key="item" class="col-2 p-0">
      ...
      <div class="modal-content px-4">
        ...
        <div class="modal-body">
          <ul style="text-align: justify !important;">
            <li v-for="law in laws" :key="law">{{ law }}</li>
          </ul>
        </div>
  ...
  mounted() {
    ...
    axios.get('https://server.elfiro.com/api/v1/orders/' + this.$route.params.id)
        .then((response) => {
          ...
          this.laws = response.data.data.Laws.laws;

          console.log(this.order.user.user_name)
          console.log('gggg',response.data.data);

          this.img1 = response.data.data.order.record.gallery[0];
          let gall = response.data.data.order.record.gallery
          gall.shift()
          this.images = gall;
          console.log('q',this.img1, this.images)
        }).catch((error) => {
      console.log(error)
    });
  },
